...
class AsignaturaTestCase ( unittest.TestCase ) :
    ...
    def test_agregar_actividad ( self ) :
        resultado = self.sorteo.agregar_actividad ( denominacionActividad = "Estudiar" , fecha=datetime(2020, 9, 28, 00, 00, 00, 00000))
    ...
    def test_verificar_almacenamiento_agregar_actividad ( self ):
        self.session=Session()
        actividad=self.session.query(Equipo).filter((Actividad.denominacionActividad == "TDD")and(Actividad.fecha == datetime ( 2021 , 9 , 25 , 00 , 00 , 00 , 00000 ))).first()
        self.assertEqual("Equipo01", actividad.denominacionEquipo)

    def test_agregar_actividad_vacio ( self ) :
        resultado = self.sorteo.agregar_actividad(denominacionActividad="",fecha ="")
